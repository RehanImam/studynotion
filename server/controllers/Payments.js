...
//         // We return 'paymentResponse' inside the 'data' key
//         res.status(200).json({
//             success: true,
...
//         res.status(500).json({ 
//             success: false, 
//             message: "Could not initiate order. Check Razorpay Keys." 
//         });
//     }
// };
...
// ✅ FIXED ENROLL STUDENTS FUNCTION
const enrollStudents = async (courses, userId, res) => {
  if (!courses || !userId) {
  ...
  for (const courseId of courses) {
    try {
      // Find the course and enroll the student in it
      // 📝 FIX: Changed 'studentsEnroled' to 'studentsEnrolled' (Added the missing 'l')
      const enrolledCourse = await Course.findOneAndUpdate(
        { _id: courseId },
